...
fn packetTypeName(ptype: u32) [:0]const u16 {
    return switch (ptype) {
        0 => w32.utf8ToUtf16("Prof"),
        1 => w32.utf8ToUtf16("LowRes"),
        2 => w32.utf8ToUtf16("HighRes"),
        4 => w32.utf8ToUtf16("LT-Prof"),
        5 => w32.utf8ToUtf16("LT-Cent"),
        20 => w32.utf8ToUtf16("FT-Cent"),
        21 => w32.utf8ToUtf16("FT-Prof"),
        22 => w32.utf8ToUtf16("HR-Comp"),
        23 => w32.utf8ToUtf16("LR-Comp"),
        24 => w32.utf8ToUtf16("LR-Type"),
        else => w32.utf8ToUtf16("?"),
    };
}
...
pub fn create(parent: w32.HWND, hInstance: w32.HINSTANCE, id: c_int) !w32.HWND {
    ...
    g_nav_prev_btn = w32.CreateWindowExW(
        0,
        w32.utf8ToUtf16Z("BUTTON"),
        w32.utf8ToUtf16Z("<"),
        w32.WS_CHILD | w32.WS_VISIBLE | w32.BS_PUSHBUTTON,
        x, 0, NAV_BUTTON_WIDTH, BUTTON_HEIGHT,
        parent, @ptrFromInt(@as(usize, @intCast(IDB_NAV_PREV))), hInstance, null,
    );
    x += NAV_BUTTON_WIDTH + CONTROL_GAP;

    g_scan_edit = w32.CreateWindowExW(
        w32.WS_EX_CLIENTEDGE,
        w32.utf8ToUtf16Z("EDIT"),
        w32.utf8ToUtf16Z(""),
        w32.WS_CHILD | w32.WS_VISIBLE | w32.ES_NUMBER | w32.ES_AUTOHSCROLL,
        x, 0, SCAN_EDIT_WIDTH, BUTTON_HEIGHT,
        parent, @ptrFromInt(@as(usize, @intCast(IDC_SCAN_EDIT))), hInstance, null,
    );
    ...
    g_nav_next_btn = w32.CreateWindowExW(
        ...
        w32.WS_CHILD | w32.WS_VISIBLE | w32.BS_PUSHBUTTON,
        x, 0, NAV_BUTTON_WIDTH, BUTTON_HEIGHT,
        parent, @ptrFromInt(@as(usize, @intCast(IDB_NAV_NEXT))), hInstance, null,
    );
    ...
    inline for (btn_labels, btn_ids, 0..) |label, bid, i| {
        const btn_hwnd = w32.CreateWindowExW(
            0,
            w32.utf8ToUtf16Z("BUTTON"),
            w32.utf8ToUtf16Z(label),
            w32.WS_CHILD | w32.WS_VISIBLE | w32.BS_AUTORADIOBUTTON,
            x + @as(c_int, @intCast(i * FILTER_BUTTON_SPACING)), 0, FILTER_BUTTON_WIDTH, BUTTON_HEIGHT,
            parent, @ptrFromInt(@as(usize, @intCast(bid))), hInstance, null,
        );
    ...
    const hwnd = w32.CreateWindowExW(
        w32.WS_EX_CLIENTEDGE,
        w32.utf8ToUtf16Z("SysListView32"),
        w32.utf8ToUtf16Z(""),
    ...
    _ = w32.ListView_SetExtendedListViewStyle(hwnd, w32.LVS_EX_FULLROWSELECT | w32.LVS_EX_GRIDLINES);
    ...
    inline for (col_names, col_widths, col_fmts, 0..) |name, width, fmt, i| {
        const name16 = w32.utf8ToUtf16(name);
        var col: w32.LVCOLUMNW = .{
            .mask = w32.LVCF_FMT | w32.LVCF_WIDTH | w32.LVCF_TEXT,
            .fmt = fmt,
            .cx = width,
            .pszText = @constCast(@ptrCast(name16.ptr)),
        ...
        _ = w32.ListView_InsertColumn(hwnd, @intCast(i), &col);
    }
...
pub fn populate(hwnd: w32.HWND, state: *app.AppState) void {
    ...
    syncFilterButtons(state.filter_ms_level);

    if (state.scans.len == 0) {
        _ = w32.ListView_SetItemCount(hwnd, 0);
    ...
    for (state.scans) |scan| {
        if (state.filter_ms_level) |level| {
    ...
    _ = w32.ListView_SetItemCount(hwnd, @intCast(filtered_count));
    updateScanEdit(state);
}
...
fn updateScanEdit(state: *app.AppState) void {
    ...
    if (state.scans.len == 0) {
        _ = w32.SendMessageW(g_scan_edit, w32.WM_SETTEXT, 0, @intCast(@intFromPtr(w32.utf8ToUtf16("").ptr)));
        return;
    }
    var buf: [32]u8 = undefined;
    const text = std.fmt.bufPrintZ(&buf, "{d}", .{state.scans[state.current_scan_index].scan_number}) catch "";
...
pub fn handleScanEditEnter(state: *app.AppState) void {
    if (g_scan_edit == null or state.scans.len == 0) return;
    var buf16: [32]u16 = undefined;
...
pub fn selectScan(hwnd: w32.HWND, scan_index: usize) void {
    ...
    const state = app.getGlobalState() orelse {
        _ = w32.ListView_SetItemState(hwnd, @intCast(scan_index), w32.LVIS_SELECTED, w32.LVIS_SELECTED);
        _ = w32.ListView_EnsureVisible(hwnd, @intCast(scan_index), 0);
        return;
    };
    if (state.filtered_indices) |fi| {
        for (fi, 0..) |idx, i| {
            if (idx == scan_index) {
                _ = w32.ListView_SetItemState(hwnd, @intCast(i), w32.LVIS_SELECTED, w32.LVIS_SELECTED);
                _ = w32.ListView_EnsureVisible(hwnd, @intCast(i), 0);
                updateScanEdit(state);
                return;
            }
        }
...
pub fn getSelectedScanIndex(hwnd: w32.HWND) ?usize {
    const sel = w32.ListView_GetNextItem(hwnd, -1, w32.LVNI_SELECTED);
    if (sel < 0) return null;
    const state = app.getGlobalState() orelse return @intCast(sel);
    if (state.filtered_indices) |fi| {
...
fn navigateToScanIndex(state: *app.AppState, scan_index: usize) void {
    if (scan_index >= state.scans.len) return;
    state.loadScan(scan_index) catch {};
    state.current_scan_index = scan_index;
    if (g_list_hwnd) |list| {
        selectScan(list, scan_index);
    }
...
pub fn handleGetDispInfo(pnmh: *w32.NMHDR) void {
    ...
    if (scan_idx >= state.scans.len) return;
    const scan = state.scans[scan_idx];
...
pub fn handleNotify(pnmh: *w32.NMHDR, state: *app.AppState) void {
    switch (pnmh.code) {
        w32.LVN_ITEMCHANGED => {
            if (g_selecting_programmatically) return;
            if (getSelectedScanIndex(pnmh.hwndFrom)) |sel_idx| {
                state.loadScan(sel_idx) catch {};
...
pub fn handleFilterCommand(state: *app.AppState, cmd_id: u16, parent: w32.HWND) void {
    ...
    state.setMsLevelFilter(level);
    // Also update chromatogram filter to match scan filter
    state.setChromatogramMsLevelFilter(level);
    ...
    if (g_list_hwnd) |list| {
        ...
        if (state.filtered_indices) |fi| {
            if (fi.len > 0) {
                ...
                selectScan(list, first_visible);
                state.loadScan(first_visible) catch {};
                state.current_scan_index = first_visible;
            }
        } else if (state.scans.len > 0) {
            selectScan(list, 0);
            state.loadScan(0) catch {};
...
fn findNextVisibleScan(state: *app.AppState, direction: i32) ?usize {
    if (state.filtered_indices) |fi| {
        ...
        if (direction > 0) {
            if (state.current_scan_index + 1 < state.scans.len) {
...
pub fn handleNavCommand(state: *app.AppState, cmd_id: u16) void {
    switch (cmd_id) {
        IDB_NAV_PREV => {
...
pub fn getScanEditHwnd() w32.HWND {
    return g_scan_edit;
}
