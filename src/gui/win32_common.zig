...
pub fn utf8ToUtf16(comptime text: []const u8) [:0]const u16 {
    return std.unicode.utf8ToUtf16LeStringLiteral(text);
}

// Helper: convert UTF-8 to UTF-16 heap-allocated string
pub fn utf8ToUtf16Alloc(allocator: std.mem.Allocator, text: []const u8) ![:0]u16 {
...
pub fn utf8ToUtf16Buf(out: []u16, text: []const u8) ![]u16 {
    var i: usize = 0;
    var j: usize = 0;
    while (i < text.len) {
...
pub fn drawTextUtf8(hdc: HDC, x: INT, y: INT, comptime text: []const u8) void {
...
pub fn getX(lParam: LPARAM) INT {
    return @intCast(lParam & 0xFFFF);
}

pub fn getY(lParam: LPARAM) INT {
...
pub fn utf8ToUtf16Z(comptime text: []const u8) [:0]const u16 {
    return utf8ToUtf16(text);
}

pub fn LOWORD(l: LPARAM) u16 {
    return @truncate(@as(u32, @intCast(l)));
}

pub fn HIWORD(l: LPARAM) u16 {
    return @truncate(@as(u32, @intCast(l)) >> 16);
}
