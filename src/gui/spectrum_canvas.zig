...
pub fn create(parent: w32.HWND, hInstance: w32.HINSTANCE, id: c_int) CreateError!w32.HWND {
    const class_name = w32.utf8_to_utf16_z("SpectrumCanvas");
...
pub fn release_cached_pens() void {
    if (g_pen_stick) |p| {
...
fn paint(hwnd: w32.HWND, state: *app.AppState) void {
    ...
    const x_min = state.view.zoom.mz_min;
    const x_max = state.view.zoom.mz_max;
...
fn drawNoData(hdc: w32.HDC, rc: w32.RECT) void {
    const msg = w32.utf8_to_utf16("No data loaded");
    var rc2 = rc;
...
fn drawStickPlot(
    ...
    for (first_idx..last_idx) |idx| {
        const x = mz_to_screen(mz[idx], plot_left, plot_w, x_min, x_range);
        const y = inten_to_screen(intensity[idx], plot_top, plot_h, y_max);
...
fn drawPeakLabels(
    ...
    for (top_peaks[0..init_count]) |peak| {
        ...
        const x = mz_to_screen(pt_mz, plot_left, plot_w, x_min, x_range);
        const y = inten_to_screen(pt_inten, plot_top, plot_h, y_max);

        var buf: [64]u8 = undefined;
        const label = std.fmt.bufPrintZ(&buf, "{d:.2}", .{pt_mz}) catch continue;
        var label16_buf: [32]u16 = undefined;
        const label16 = w32.utf8_to_utf16_buf(&label16_buf, label) catch continue;
...
fn drawAxes(
    ...
    while (x_val <= x_max) : (x_val += x_step) {
        const sx = mz_to_screen(x_val, plot_left, plot_w, x_min, x_range);
    ...
    while (y_val <= y_max) : (y_val += y_step) {
        const sy = inten_to_screen(y_val, plot_top, plot_h, y_max);
    ...
    // Axis titles
    const x_title = w32.utf8_to_utf16("m/z");
...
fn drawInfoText(hdc: w32.HDC, spectrum: *const advanced.Spectrum, rc: w32.RECT, state: *app.AppState) void {
    ...
    const info = std.fmt.bufPrintZ(&buf, "Points: {d} | m/z: {d:.2} - {d:.2} | Max: {e:.2}", .{ spectrum.point_count(), spectrum.get_mz_min(), spectrum.get_mz_max(), spectrum.get_intensity_max() }) catch return;
    ...
    const scan16 = w32.utf8_to_utf16_buf(&scan16_buf, scan_info) catch scan16_buf[0..0];
    ...
    if (scan.filter_string) |fs| {
        ...
        const filter16 = w32.utf8_to_utf16_buf(&filter16_buf, filter_info) catch filter16_buf[0..0];
    ...
    if (g_selected_peak_idx) |pi| {
        if (pi < spectrum.mz.len) {
            ...
            if (f.flags.reference) {
                flag_buf[flag_len] = 'R';
                flag_len += 1;
            }
            if (f.flags.exception) {
                flag_buf[flag_len] = 'E';
                flag_len += 1;
            }
            if (f.flags.modified) {
            ...
            const peak16 = w32.utf8_to_utf16_buf(&peak16_buf, peak_info) catch peak16_buf[0..0];
...
fn mz_to_screen(mz: f64, plot_left: i32, plot_w: i32, x_min: f64, x_range: f64) i32 {
    const scale = @as(f64, @floatFromInt(plot_w)) / x_range;
    return plot_left + @as(i32, @intFromFloat(@mulAdd(f64, mz - x_min, scale, 0.0)));
}

fn inten_to_screen(inten: f64, plot_top: i32, plot_h: i32, y_max: f64) i32 {
...
fn handleMouseDown(hwnd: w32.HWND, state: ?*app.AppState, x: u16, y: u16) void {
    ...
    if (state) |s| {
        ...
        const x_min = s.view.zoom.mz_min;
        const x_max = s.view.zoom.mz_max;
        const x_range = if (x_max > x_min) x_max - x_min else 1.0;
...
fn handleMouseWheel(hwnd: w32.HWND, state: *app.AppState, wParam: w32.WPARAM, lParam: w32.LPARAM) void {
    ...
    const x_range = state.view.zoom.mz_span();

    if (keys & 0x0004 != 0) { // MK_SHIFT — pan left/right
        const pan_factor = @divTrunc(-delta, @as(i32, w32.WHEEL_DELTA));
        state.view.zoom.pan_by(x_range * -@as(f64, @floatFromInt(pan_factor)) * 0.1);
    } else {
        // Zoom centered on cursor position
        if (pt.x >= plot_left and pt.x <= plot_right) {
            const click_mz = state.view.zoom.mz_min + (@as(f64, @floatFromInt(pt.x - plot_left)) / @as(f64, @floatFromInt(plot_w))) * x_range;
            const factor: f64 = if (delta > 0) 0.85 else 1.18;
            state.view.zoom.zoom_around(click_mz, factor);
        } else {
            const factor: f64 = if (delta > 0) 0.85 else 1.18;
            const center_mz = (state.view.zoom.mz_min + state.view.zoom.mz_max) / 2.0;
            state.view.zoom.zoom_around(center_mz, factor);
        }
    }
    invalidate(hwnd);
}
