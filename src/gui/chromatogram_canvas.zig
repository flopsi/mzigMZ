...
pub fn create(parent: w32.HWND, hInstance: w32.HINSTANCE, id: c_int) !w32.HWND {
    const class_name = w32.utf8ToUtf16Z("ChromatogramCanvas");
...
pub fn setState(hwnd: w32.HWND, state: *app.AppState) void {
    _ = w32.SetWindowLongPtrW(hwnd, w32.GWLP_USERDATA, @bitCast(@as(usize, @intFromPtr(state))));
}

pub fn setChromatogramType(typ: ChromatogramType) void {
    g_chromatogram_type = typ;
    g_zoom_active = false;
}

fn releaseCachedPens() void {
...
fn paint(hwnd: w32.HWND, state: *app.AppState) void {
    ...
    const info = std.fmt.bufPrintZ(&buf, "Points: {d} | RT: {d:.2} - {d:.2} min | Max: {e:.2}",
        .{ visible_count, rt_min, rt_max, inten_max }) catch return;
...
fn drawNoData(hdc: w32.HDC, rc: w32.RECT) void {
    const msg = w32.utf8ToUtf16("No chromatogram data");
    var rc2 = rc;
...
fn drawAxes(
    ...
    // Axis titles
    const x_title = w32.utf8ToUtf16("RT (min)");
