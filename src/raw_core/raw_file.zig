...
/// Scan trailer metadata extracted from the RAW file.
...
// ============================================================================
// ScanEvent structures (from ThermoFisher.CommonCore.RawFileReader)
// ============================================================================

/// ScanEventInfoStruct (136 bytes, file rev >= 65)
/// Layout verified against C# Marshal.SizeOf(typeof(ScanEventInfoStruct)) = 136
/// C# uses [StructLayout(LayoutKind.Sequential, Pack = 8)] — default for sequential structs.
/// Padding is inserted so multi-byte fields start at offsets aligned to their size.
///
/// Full field layout from C# decompiled source:
///   offsets 0-11:   basic flags (tightly packed, no alignment needed)
///   offset 12-15:   padding to align DetectorValue (f64) to 8
///   offset 16-23:   DetectorValue (f64)
///   offset 24:      SourceFragmentationType (u8)
///   offset 25-27:   padding to align ScanTypeIndex (i32) to 4
///   offset 28-31:   ScanTypeIndex (i32)
///   offset 32:      Wideband (u8)
///   offset 33-35:   padding to align AccurateMassType (enum=int) to 4
///   offset 36-39:   AccurateMassType (u32 — C# enum = int)
///   offset 40-46:   mass analyzer flags (tightly packed)
///   offset 47:      padding to align MultiPhotonDissociation (f64) to 8
///   offset 48-55:   MultiPhotonDissociation (f64)
///   offset 56:      ElectronCaptureDissociationType (u8)
///   offset 57-63:   padding to align ElectronCaptureDissociation (f64) to 8
///   offset 64-71:   ElectronCaptureDissociation (f64)
///   offset 72-73:   PhotoIonization (u8), PulsedQDissociationType (u8)
///   offset 74-79:   padding to align PulsedQDissociation (f64) to 8
///   offset 80-87:   PulsedQDissociation (f64)
///   offset 88:      ElectronTransferDissociationType (u8)
///   offset 89-95:   padding to align ElectronTransferDissociation (f64) to 8
///   offset 96-103:  ElectronTransferDissociation (f64)
///   offset 104:     HigherEnergyCIDType (u8)
///   offset 105-111: padding to align HigherEnergyCID (f64) to 8
///   offset 112-119: HigherEnergyCID (f64)
///   offset 120-130: trailing flags (tightly packed)
...
/// Read scan trailer from memory-mapped file.
/// The trailer is a structured record at trailer_offset from the start of the file.
pub fn readScanTrailer(
    allocator: std.mem.Allocator,
    mm: std.Io.File.MemoryMap,
    trailer_offset: i32,
) RawResolveError!ScanTrailer {
    if (trailer_offset <= 0) {
        return .{ .filter_string = null, .ms_level = 1, .charge_state = 0, .precursor_mz = 0 };
    }
    const off: usize = @intCast(trailer_offset);
    if (off + 8 > mm.memory.len) return RawResolveError.Truncated;
    ...
    if (num_entries < 0 or num_entries > 1000) return RawResolveError.InvalidRawFileInfo;
    ...
    while (i < num_entries) : (i += 1) {
        ...
        switch (label) {
            9 => { // Filter string
                if (value > 0 and value < MAX_STRING_CHARS) {
